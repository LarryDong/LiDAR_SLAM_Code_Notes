...
void MAP_MANAGER::MapIncrement(const pcl::PointCloud<PointType>::Ptr& laserCloudCornerStack,
  ...
  for(int i = 0; i < laserCloudNum; i++){
    ...
    laserCloudSurf_for_match[i] = *laserCloudSurfArray[i];        //~ 这些变量在Estimate时用到了
  ...
  bool CornerChangeFlag[laserCloudNum] = {false};   //~ 标记某个Cube是否发生了变化。
  ...
  for (int i = 0; i < laserCloudCornerStackNum; i++) {
    ...
    if (cubeI >= 0 && cubeI < laserCloudDepth &&
      ...
      laserCloudCornerArray[cubeInd]->push_back(pointSel);    //~ 将特征放入对应的Cube并修改 changFlag
  ...
  t3 = clock();
  // 如果某个Cube发生了变化，重建这个Cube对应的kdtree
  ...
  for(int i = 0; i < laserCloudNum; i++){
    CornerKdMap_copy[i] = *laserCloudCornerKdMap[i];    //~ 这个copy没有用到
...
void MAP_MANAGER::MapMove(const Eigen::Matrix4d& transformTobeMapped){
  ...
  //~ I和i对应Map的depth(最大21)，如果i小于8(或大于21-8)，则进行平移
  //~ 在i过于靠边时，将(laserCloudDepth - 1)的点丢弃，然后全部从index_a <- index_b 移动，将更小的i移到更大的i位置，同时在最新空出来的位置i=0进行清空。
  //~ 清空时赋予了 laserCloudCubeCornerPointer，紧接着这个pointer被clear了，相当于这个位置的点云是空的。但Kd-tree的pointer没有清空，只是清空了点云数据，推测在重新建立kdtree时根据新点云可以。
  while (centerCubeI < 8) {
