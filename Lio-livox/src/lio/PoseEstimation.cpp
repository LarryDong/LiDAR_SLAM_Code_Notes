...
bool TryMAPInitialization() {
  ...
  double info_g = std::fabs(9.805 - average_acc.norm());  // 没有用到
  ...
  //~ AddResidualBlock (CostFunction* cost_function, LossFunction* loss_function, const vector<double*>& parameter_blocks)
  ...
  int v_size = lidarFrameList->size();      // 尺寸应该是 windowSize
  ...
  prior_v[0] = prior_v[1];    // i=0的速度设定为和1的一样。
...
void process(){
  ...
  Eigen::Matrix3d delta_Rl = Eigen::Matrix3d::Identity();     //~ 这个是Lidar系下的位姿增量。纯Lidar时用于估计运动参数，去畸变；
  Eigen::Vector3d delta_tl = Eigen::Vector3d::Zero();
	Eigen::Matrix3d delta_Rb = Eigen::Matrix3d::Identity();     //~ 应该是IMU的增量；
  ...
  while(ros::ok()){
    ...
    if(newfullCloud){
      ...
      if(IMU_Mode > 0 && time_last_lidar > 0){      // 如果有IMU和lidar，则进行数据同步
...
if(IMU_Mode > 1 && !LidarIMUInited){		// 紧耦合，且没有初始化，才有可能进行初始化操作。
	// pushCount 的一些计算
	if(!LidarIMUInited && ...)
		LidarIMUInited = ture
}
// */
	    ...
	    if(!vimuMsg.empty()){     // --> 1. IMU 有数据
	    	if(!LidarIMUInited) {   // --> 1.1 IMU / lidar 没有初始化，则进行初始化操作。
			    ...
			    delta_Rl = exTlb.topLeftCorner(3, 3) * delta_Rb * exTlb.topLeftCorner(3, 3).transpose();    //~ 将IMU的旋转转到lidar的旋转。

			    // predict current lidar pose
			    lidarFrame.P = transformAftMapped.topLeftCorner(3,3) * delta_tb
			                   + transformAftMapped.topRightCorner(3,1);                  //~ 从这个表达式看，AftMapped是base的坐标系的位姿。
			    Eigen::Matrix3d m3d = transformAftMapped.topLeftCorner(3,3) * delta_Rb;
			    lidarFrame.Q = m3d;

			    lidar_list.reset(new std::list<Estimator::LidarFrame>);   // 每次都reset了，这个lidar_list只有一个lidarFrame
			    lidar_list->push_back(lidarFrame);
		    }else{                  // --> 1.2 IMU 已经初始化了（紧耦合模式），用于计算上次lidar到这次lidar过程中IMU预积分得到的增量，并转到lidar下用于去畸变。
			    // if get IMU msg successfully, use pre-integration to update delta lidar pose
			    lidarFrame.imuIntegrator.PushIMUMsg(vimuMsg);   // 把这段时间所有的IMU都放进去
			    lidarFrame.imuIntegrator.PreIntegration(lidarFrameList->back().timeStamp, lidarFrameList->back().bg, lidarFrameList->back().ba);

			    const Eigen::Vector3d& Pwbpre = lidarFrameList->back().P;     // 这是更新之前，上一组lidarFrame最终得到的PQV
			    const Eigen::Quaterniond& Qwbpre = lidarFrameList->back().Q;
			    const Eigen::Vector3d& Vwbpre = lidarFrameList->back().V;

			    const Eigen::Quaterniond& dQ =  lidarFrame.imuIntegrator.GetDeltaQ();   // 这是预积分得到的增量部分
			    ...
			    lidarFrame.Q = Qwbpre * dQ;     // 更新当前lidarFrame的QPV
			    ...
			    Eigen::Quaterniond Qwlpre = Qwbpre * Eigen::Quaterniond(exRbl);   // 计算lidar系的增量
			    ...
			    delta_tl = Qwlpre.conjugate() * (Pwl - Pwlpre);     //~ 紧耦合下IMU的加速度漂移应该去掉了，所以积分平移应该是可以的。
			    delta_Rb = dQ.toRotationMatrix();
			    delta_tb = dP;

			    lidarFrameList->push_back(lidarFrame);    // 仅有这种模式，紧耦合，lidarFrameList的大小才不是0，是window_size
			    lidarFrameList->pop_front();
			    lidar_list = lidarFrameList;
	    	}
	    }else{
	    	if(LidarIMUInited)    // 2.1 没有IMU数据，但已经初始化了，意味着没有数据
	    	  break;
	    	else{                 // 2.2 没有IMU数据，没有初始化（只有IMU_MODE=2即LIO紧耦合，才会初始化）。所以这种模式只有 LiDAR做 LO
			    // predict current lidar pose
	    ...
	    RemoveLidarDistortion(laserCloudFullRes, delta_Rl, delta_tl);             //~ 去畸变参数来自上次运动增量(无IMU)，或IMU预积分； delta_Rl 和 tl 是lidar坐标系的量。
      ...
      //~ 2023.09.04 lidar_list->的Q和P是IMU的，ToBeMapped是IMU的，AftMapped是lidar的；
	    Eigen::Matrix4d transformTobeMapped = Eigen::Matrix4d::Identity();            //~ lidar在world的位姿（确认）
	    transformTobeMapped.topLeftCorner(3,3) = lidar_list->front().Q * exRbl;       //~ Q应该是IMU的Q，因此通过Rbl(从body到lidar)转到lidar系？但是是右乘？？
	    ...
	    transformAftMapped.topLeftCorner(3,3) = lidar_list->front().Q.toRotationMatrix();       //~ AftMapped 是lidar or IMU 的坐标呢？（目前怀疑是IMU的）
	    transformAftMapped.topRightCorner(3,1) = lidar_list->front().P;

	    // publish odometry rostopic
	    pubOdometry(transformTobeMapped, lidar_list->front().timeStamp);                    //~ odom发布的是lidar的位姿，ToBeMapped是lidar的
      ...
      for (int i = 0; i < laserCloudFullResNum; i++) {
        ...
        MAP_MANAGER::pointAssociateToMap(&lidar_list->front().laserCloud->points[i], &temp_point, transformTobeMapped);   //~ 更加验证了 ToBeMapped 是lidar系。
      ...
      pubFullLaserCloud.publish(laserCloudMsg);
	    ...
	    if(IMU_Mode > 1 && !LidarIMUInited){      // 紧耦合的初始化。
          ...
          if (lidarFrameList->size() == int(WINDOWSIZE / 1.5)) {
					  startTime = lidarFrameList->back().timeStamp;       // 这个值没有用到
			    }
...
int main(int argc, char** argv)
{
  ...
  ros::param::get("~IMU_Mode",IMU_Mode);
	...
	ros::param::get("~Extrinsic_Tlb",vecTlb);     //~ vecTlb是从lidar到IMU系的变换，即IMU在lidar系的表示
