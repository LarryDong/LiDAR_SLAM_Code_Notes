...
//~ noreturn函数，是告知编译器合格函数死活不会返回任何值，可以进行相关优化。一般用于：程序终止前的处理，或while循环
[[noreturn]] void Estimator::threadMapIncrement(){
  ...
  while(true){
    ...
    if(!laserCloudCornerForMap->empty()){
      ...
      map_update_ID ++;
      //~ 将 lidar 系下的特征坐标，变化到 Map 下。注意 laserCloudCornerForMap 这些特征，在局部地图也用过一次更新。
      map_manager->featureAssociateToMap(laserCloudCornerForMap,
                                         ...
                                         transformForMap);      //~ laserCloudCorner 是const指针，指针不能变，但指向的内容有更新？
      ...
      *laserCloudCorner_to_map += *laserCloudCorner;        //~ 将ForMap的特征转到Map中，然后并到laserCloudCorner_to_map中。累加是为了 map_skip_frame 前的保存，避免频繁插入。
...
void Estimator::vector2double(const std::list<LidarFrame>& lidarFrameList){
  ...
  for(const auto& l : lidarFrameList){
    Eigen::Map<Eigen::Matrix<double, 6, 1>> PR(para_PR[i]);   // 待优化的P和R参数，6维的。
    PR.segment<3>(0) = l.P;
...
void Estimator::EstimateLidarPose(std::list<LidarFrame>& lidarFrameList,
  ...
  transformTobeMapped.topLeftCorner(3,3) = lidarFrameList.back().Q * exRbl;         //~ 后面又再次赋值了，没有用这里。
...
void Estimator::Estimate(std::list<LidarFrame>& lidarFrameList,
  ...
  Eigen::Matrix3d exRbl = exTlb.topLeftCorner(3,3).transpose();
  Eigen::Vector3d exPbl = -1.0 * exRbl * exTlb.topRightCorner(3,1);
  kdtreeCornerFromLocal->setInputCloud(laserCloudCornerFromLocal);      //~ laserCloudCornerFromLocal
  ...
  for(int iterOpt=0; iterOpt<max_iters; ++iterOpt){
    ...
    if(windowSize == SLIDEWINDOWSIZE) {     // 如果只有一个，即纯lidar，则不需要任何异常值处理的function了
      loss_function = NULL;
    } else {
      loss_function = new ceres::HuberLoss(0.1 / IMUIntegrator::lidar_m);
    }
    ...
    // add IMU CostFunction  ~ 只有lidar时不加入，因为windowSize应该是1
    for(int f=1; f<windowSize; ++f){
      auto frame_curr = lidarFrameList.begin();
      std::advance(frame_curr, f);      // 将frame_curr 这个迭代器，向后移动 f 个位置；
      problem.AddResidualBlock(Cost_NavState_PRV_Bias::Create(frame_curr->imuIntegrator,
                                                              ...
                                                              Eigen::LLT<Eigen::Matrix<double, 15, 15>>
                                                                      ...
                                                                      .matrixL().transpose()),      // Eigen::LLT，cholesky分解
                               nullptr,
                               para_PR[f-1],
                               para_VBias[f-1],
                               para_PR[f],
    ...
    if(windowSize == SLIDEWINDOWSIZE) {
      ...
      if(iterOpt == 0){
        ...
        for(int f=0; f<windowSize; ++f){
          int cntFtu = 0;       // count FeaTUre 的缩写？
          for (auto &e : edgesLine[f]) {
            if(std::fabs(vLineFeatures[f][cntFtu].error) > 1e-5){     // error 是在 processPointToLine 计算得到的。
              problem.AddResidualBlock(e, loss_function, para_PR[f]); // 只对误差比较大的项进行优化？
              vLineFeatures[f][cntFtu].valid = true;
            }else{
              vLineFeatures[f][cntFtu].valid = false;
            }
    ...
    if (deltaR < 0.05 && deltaT < 0.05 || (iterOpt+1) == max_iters){    // 优化收敛或达到最大值
      ROS_INFO("Frame: %d\n",frame_count++);
      if(windowSize != SLIDEWINDOWSIZE) break;            // 这里直接break
...
void Estimator::MapIncrementLocal(const pcl::PointCloud<PointType>::Ptr& laserCloudCornerStack,         //~ 将lidar系下的点，转化到地图中。
  ...
  localCornerMap[Id]->clear();        //~ 清空当前局部区域的地图，进行更新。ID是0~50循环的
  ...
  //~ WindowSize是50，提取50个最近（时间上）的localCornerMap，然后加一起组成 `laserCloudCornerFromLocal`，用于estimate
  for (int i = 0; i < localMapWindowSize; i++) {
    *laserCloudCornerFromLocal += *localCornerMap[i];   //~ 再把局部地图加上去。但是除了[Id]以外的部分应该不变，但是在调用 MapIncrementLocal 前clear了一次，不知道为啥。
  ...
  laserCloudCornerFromLocal = temp;                   //~ laserCloudCornerFromLocal 这个变量，用于localMap的更新，也用于了整个Map的Increment
