...
struct Cost_Initialization_Prior_R
{
	...
	bool operator()( const T *r_wg_, T *residual) const {
		// Eigen::Map 是 Eigen 库中的一个模板类，它允许你将 Eigen 数据结构（如向量或矩阵）映射到已存在的内存中。
		// 这意味着你可以使用 Eigen 的接口和算法来操作存储在普通数组或其他数据结构中的数据，而无需复制数据。
		...
		Sophus::SO3<T> d_R = SO3_R_wg.inverse() * prior_R_wg;			// 残差定义：r_wg_ 和 prior 的差
		...
		eResiduals.applyOnTheLeft(sqrt_information.template cast<T>());		// sqrit_info(3x3) * eResiduals(3x1), .template cast<T>() 将 sqrt_information 矩阵中的每个元素转换为类型 T
