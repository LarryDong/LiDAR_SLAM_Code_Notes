...
namespace esekfom
{
	...
	struct dyn_share_datastruct
	{
		...
		Eigen::Matrix<double, Eigen::Dynamic, 1> h;				   //残差	(公式(14)中的z)  ~维度为Mx1，M是特征点数量，动态的。
		Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> h_x; //雅可比矩阵H (公式(14)中的H)	//~ 24维
	};
	...
	public:
		...
		void predict(double &dt, Eigen::Matrix<double, 12, 12> &Q, const input_ikfom &i_in)
		{
			...
			P_ = (f_x_)*P_ * ( ).transpose() + (dt * f_w_) * Q * (dt * f_w_).transpose(); //传播协方差矩阵，即公式(8)
		}
...
#ifdef MP_EN
			omp_set_num_threads(MP_PROC_NUM);			//~ 近邻点的搜索，开启多线程 TODO: 查一下函数
#pragma omp parallel for
			...
			for (int i = 0; i < feats_down_size; i++) //遍历所有的特征点
			{
				...
				PointType point_world;					///~ feature点转到世界坐标系
				...
				if (ekfom_data.converge)				//~ 如果收敛，则寻找新的匹配；否则依然用上次的。
				{
					//寻找point_world的最近邻的平面点
					ikdtree.Nearest_Search(point_world, NUM_MATCH_POINTS, points_near, pointSearchSqDis);
					//判断是否是有效匹配点，与loam系列类似，要求特征点最近邻的地图点数量>阈值，距离<阈值  满足条件的才置为true		~ 满足：足够多的近邻点，且最大距离不大于5m
				...
				{
					float pd2 = pabcd(0) * point_world.x + pabcd(1) * point_world.y + pabcd(2) * point_world.z + pabcd(3); //当前点到平面的距离	~ |Ax+By+Cz+D|/|模长|，已归一化了
			...
			// 雅可比矩阵H和残差向量的计算
			ekfom_data.h_x = MatrixXd::Zero(effct_feat_num, 12);	//~ 12的含义：把原来的24维拆开了进行计算，更快。H的前12维，而不是24。
			ekfom_data.h.resize(effct_feat_num);		//~ h是残差
			...
			for (int i = 0; i < effct_feat_num; i++)
			{
				...
				V3D A(point_I_crossmat * C);			//~ 点转到IMU坐标系后的反对称矩阵，对应Fast-lio课件中的公式(13)的第一项。注意形式上交换了左右矩阵。公式是-R*C^，这里是R^*C
				if (extrinsic_est)
				{
					...
					//~ 状态量前3维是法向量（状态量定义的前三维是平移量，所以求导的前3是n；后面的是旋转量的求导）Fast-Lio课件的(13)
		...
		// ESKF
		//~ R 是协方差矩阵中的元素，认为是一致的，因此直接用R, `Nearest_Points`用于后续建图
												...
												KD_TREE<PointType> &ikdtree, vector<PointVector> &Nearest_Points, int maximum_iter, bool extrinsic_est)
		{
			...
			dyn_share.valid = true;				//~ 特征点是否符合要求；
			dyn_share.converge = true;			//~ 是否收敛
			int t = 0;
			state_ikfom x_propagated = x_; //这里的x_和P_分别是经过正向传播后的状态量和协方差矩阵，因为会先调用predict函数再调用这个函数	~公式(18)的x_k^
			cov P_propagated = P_;

			vectorized_state dx_new = vectorized_state::Zero(); // 24X1的向量	~更新量，广义+的后半部分
			...
			for (int i = -1; i < maximum_iter; i++) // maximum_iter是卡尔曼滤波的最大迭代次数
			{
				...
				auto K_front = (HTH / R + P_.inverse()).inverse();			//~ R是协方差的对角线元素，标量，直接除相当于求逆了。
				...
				KH.block<24, 12>(0, 0) = K * H;							//~ 只有前面的12维需要用到。
				...
				for (int j = 0; j < 24; j++)
				{
					if (std::fabs(dx_[j]) > epsi) //如果dx>epsi 认为没有收敛		//~ epsi=0.001
					{
						dyn_share.converge = false;	//~ 如果没有收敛，则不进行新的点-面近邻点寻找；收敛，则进行下一次。
						break;
					}
				}
