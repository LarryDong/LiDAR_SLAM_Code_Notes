...
void livox_pcl_cbk(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
    ...
    if (time_sync_en && !timediff_set_flg && abs(last_timestamp_lidar - last_timestamp_imu) > 1 && !imu_buffer.empty())
    {
        ...
        timediff_lidar_wrt_imu = last_timestamp_lidar + 0.1 - last_timestamp_imu;           //~ TODO: 这里的 (+0.1) 是为什么？
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    ...
    if (abs(timediff_lidar_wrt_imu) > 0.1 && time_sync_en)      //~ 时间同步如果启动，则加上误差。
    {
        msg->header.stamp = ros::Time().fromSec(timediff_lidar_wrt_imu + msg_in->header.stamp.toSec());
    }

    msg->header.stamp = ros::Time().fromSec(msg_in->header.stamp.toSec() - time_diff_lidar_to_imu);     //~ lidar和IMU的误差同步
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    if (!lidar_pushed)
    {
        ...
        else if (meas.lidar->points.back().curvature / double(1000) < 0.5 * lidar_mean_scantime)    //~ 数据量比较少，有遮挡等。
        {
            lidar_end_time = meas.lidar_beg_time + lidar_mean_scantime;
        }
        else
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        {
            double t00 = omp_get_wtime();               //~ 获取当前时刻的时间（代码最后部分用于计算耗时）,<omp.h>
            ...
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;    //~ 全局坐标，lidar在世界系的坐标，用于后续更新地图
            state_point = kf.get_x();

            flg_EKF_inited = (Measures.lidar_beg_time 0- first_lidar_time) < INIT_TIME ? false : true;   //~ 0.1s 完成
            ...
            Nearest_Points.resize(feats_down_size); //存储近邻点的vector        ~每个点都存储近邻点
            ...
            state_point = kf.get_x();
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;    //~ TODO:
