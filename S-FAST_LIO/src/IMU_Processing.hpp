...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)
{
  ...
  for (const auto &imu : meas.imu)    //根据所有IMU数据，计算平均值和方差
  {
    ...
    cov_acc = cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - mean_acc)  / N;   //~ `cwiseProduct` 逐个元素相乘
    cov_gyr = cov_gyr * (N - 1.0) / N + (cur_gyr - mean_gyr).cwiseProduct(cur_gyr - mean_gyr)  / N / N * (N-1);

    N ++;
  }
  
  state_ikfom init_state = kf_state.get_x();        //在esekfom.hpp获得x_的状态   //~ 这里应该是第一次获得的 x
  init_state.grav = - mean_acc / mean_acc.norm() * G_m_s2;    //得平均测量的单位方向向量 * 重力加速度预设值   //~ 避免不同厂商的IMU的输入尺度不一样
  
  init_state.bg  = mean_gyr;      //角速度测量作为陀螺仪偏差
  init_state.offset_T_L_I = Lidar_T_wrt_IMU;      //将lidar和imu外参传入    //~ FAST-LIO2多优化了LiDAR和IMU的外参；
...
//反向传播
//~ 在IMU和lidar数据提取打包的时候，最后一个IMU是在lidar结束之后的。
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  //~ 这个第一个，是上一次lidar之后的尾部，还是这个新lidar扫描的第一个？ --> 应该是上一次lidar的最后一个。6:22s视频，但问题是为什么设置为0？ --> 下方 dt 计算时，如果发现是小于 last_lidar_end_time_了，则用lidar的时间戳计算，所以没关系。
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)      //~ 这里计算的是这个point对应的前(head)后(tail)两个IMU，插值得到i的R，T后，补偿。
    {
      ...
      //~ 这个去畸变很奇怪的公式：点在去畸变后在lidar末尾时刻lidar系的坐标再转到world系，等于去畸变前扫到该点时位于world系的坐标。
      //~ imu_state.rot(lidar末尾时刻在world系)*(R_L^I*p_compensate+T_L^I)+imu_state.pos(lidar末尾时刻的平移) = R_i*(R_L^I*Pi_in_lidarEnd+T_L^I)+p_i_pose，移项得到p_conpensate并带入(T_ei=Pi_in_lidarEnd-imu_state.pos)
...
void ImuProcess::Process(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI::Ptr &cur_pcl_un_)
{
  ...
  if (imu_need_init_)   
  {
    ...
    IMU_init(meas, kf_state, init_iter_num);  //如果开头几帧  需要初始化IMU参数 //~ 初始时，静止IMU，否则会不准

    imu_need_init_ = true;
    
    last_imu_ = meas.imu.back();

    state_ikfom imu_state = kf_state.get_x();   //~ TODO: 没有用到？

    if (init_iter_num > MAX_INI_COUNT)
    {
      cov_acc *= pow(G_m_s2 / mean_acc.norm(), 2);    //~ 白赋值了？下面又给cov_acc_scale了？
